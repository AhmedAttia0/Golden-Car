...
export default function ProfileSidebar({ className }) {
  ...
  return (
    <div
      className={`w-full md:w-72 lg:w-96 bg-white shadow-md rounded-xl p-6 flex flex-col items-center ${className}`}
    >
      {/* Profile Image */}
      ...
      <ul className="w-[60%] mx-auto mt-4 flex flex-col gap-4 mr-10">
        <li
          ...
          className={`flex items-center gap-3 cursor-pointer hover:text-purple-600 transition ${isActive(
            "/profile"
          )}`}
        >
        ...
        >
          ...
          <span className="text-red-600 text-lg">تسجيل الخروج</span>
        </li>
      </ul>
    </div>
  );
